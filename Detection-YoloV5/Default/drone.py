...
s1 = time_sync()
@torch.no_grad()
def run( weights=ROOT / '/home/cal-display1/Documents/hdd2/Rabia/Yolov5/yolov5/runs/train/exp7/weights/best.pt', 
        source=ROOT / '/home/cal-display1/Downloads/Drone/drone22.mp4',  #file, 0 for webcam
 ...
 #label = None if hide_labels else (names[c] if hide_conf else f'{names[c]}') #If we can write 'drone' on frame
        ...
        #LOGGER.info(f'{s}Process Prediction and Write Result time: ({s6 - s5:.3f}s)')
    s2 = time_sync()
    LOGGER.info(f'{s}Function time: ({s2 - s1:.3f}s)')
    LOGGER.info(f'{s}Load Model time: ({s4 - s3:.3f}s)') 
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / '/home/cal-display1/Documents/hdd2/Rabia/Yolov5/yolov5/runs/train/exp7/weights/best.pt', help='model path(s)')
    parser.add_argument('--source', type=str, default=ROOT / '/home/cal-display1/Downloads/Drone/drone22.mp4', help='file/dir/URL/glob, 0 for webcam')
